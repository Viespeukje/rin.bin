...
message.channel.send({embed}).catch(err => console.log("\x1b[31m%s\x1b[0m", `ERROR: Tried to post an operator reset. \n>>${err}`));
    ...
    // const embed = new Discord.RichEmbed()
    //     .addField(`Operator Mute`, `Operator <@${message.author.id}> reset channel ${message.channel}.`)
    //     .setColor(15844367)
    //     .setTimestamp()
    //     .setFooter("I've been a good girl, right..?", client.user.avatarURL);
    // message.guild.channels.find("name", "staff_lounge").send({embed}).catch(err => console.log("\x1b[31m%s\x1b[0m", `ERROR: Tried to post an operator reset. \n>>${err}`));
